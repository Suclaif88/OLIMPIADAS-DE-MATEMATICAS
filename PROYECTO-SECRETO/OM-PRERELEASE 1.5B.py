...
def reproducir_audio():
    pygame.mixer.init()
    pygame.mixer.music.load("RECURSOS/fondo.mp3")
...
preguntas_respondidas = set()  # Para llevar un registro de las preguntas respondidas
...
def abrir_ventana_juego(grado):
    ...
    # Restablecer el puntaje al iniciar el juego
    puntaje_actual = 0

    # Inicializar la lista de botones de opciones
    botones_opciones = []

    # Restablecer el conjunto de preguntas respondidas
    ...
    def avanzar_pregunta():
        ...
        # Si no hay preguntas disponibles o se han respondido todas las preguntas
         for boton in botones_opciones:
            boton.config(state=tk.DISABLED)
